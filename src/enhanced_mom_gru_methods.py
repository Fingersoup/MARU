"""
Core methods for Enhanced MoM-GRU Cell.

This file contains the main computational methods that will be added to the
EnhancedMoMGRUCell class. Split into separate file due to size constraints.
"""
...
def forward(self, input_tensor: torch.Tensor, hidden: Optional[torch.Tensor] = None,
    ...
    """
    Forward pass of the Enhanced MoM-GRU Cell.
    ...
    # Step 6: Update importance scores (if gradients are available)
    if memory_state.requires_grad and self.training:
        # This will be called during backward pass via hooks
        pass

    # Step 7: Collect monitoring statistics
    ...
    # Step 8: Increment step counter for scheduling
    if self.training:
